...
useEffect(()=>{
  if(theme==="dark"){
    ...
    const getVideoList=async ()=>{
        const videoData=await fetch(VIDEO_LIST_URL + CATAGORY_FILTER_PREFIC+category);
        const videoJson= await videoData.json();
        setSearchVideosList(videoJson.items);
    }
    return (searchVideosList.length>0) && (
    <div className='dark:bg-black dark:text-white h-[100vh]'>
      {searchVideosList.map(video =>(<Link to={"/watch?v="+video?.id?.videoId} key={video?.id?.videoId} > <SearchVideoCard info={video}/> </Link>))};
    </div>
  )
}
